...
最后完成v3，实现了可玩性，完整的操作。
优化：
> 还有一个小请求：如果这些npc我说：跟我走/一起去吃饭/一起去看看公告（对话里）之类的，然后这些npc就能够实现就好了。不需要专门的按钮，只需要说一声

对fable：
> 现在实现的很有意思了，主要是一些优化。有些AI npc幻觉严重，捏造不存在的东西。然后感觉现在的对话页面也不直观，记录在右侧小小的一坨，根本看不清什么。再写写优化的goal吧

最后：
> 现在已经交付了。最后感觉就是，作为一个游戏，它有一个新的玩家来玩，存档管理？（本地吗？）请你翻阅实现，确认最终存档管理的最佳设计，给我方案。

修复bug：
> (提供截图)有一个v3.2就带来的前端显示bug，在对话的时候可能出现这样的画面：[Image #1] ，可能表现为联想聊天过长导致挤占窗口。请修复。